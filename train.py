...
from networks import define_G, define_UNet, define_BG, define_D, define_Z, define_content, GANLoss, get_scheduler, update_learning_rate
...
parser.add_argument('--dataset_syn', type=str, default='SYNTHESIZED', help='synthesized dataset')
...
parser.add_argument('--sparse_w', type=float, default=0.0012, help='weight of sparse penalty')
parser.add_argument('--content_w_all', type=float, default=200.0, help='weight of perceptual loss')
parser.add_argument('--BCE_w', type=float, default=1.0, help='weight of BCE loss')
...
net_mask = define_UNet(opt.input_nc, opt.output_nc, opt.ngf, 'instance', False, 'normal', 0.02, gpu_id=device)
net_bg = define_BG(opt.noise_nc, opt.output_nc, 'normal', 0.02, gpu_id=device)
if opt.loss == 'wgan-gp':
...
optimizer_mask = optim.Adam(net_mask.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
optimizer_bg = optim.Adam(net_bg.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
...
net_mask_scheduler = get_scheduler(optimizer_mask, opt)
net_bg_scheduler = get_scheduler(optimizer_bg, opt)
...
for epoch in range(opt.epoch_count, opt.niter + 1):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        back = back.repeat(opt.batch_size, 1, 1, 1)
        ...
        if opt.loss == 'wgan-gp':
            ...
            loss_g = criterionGAN(pred_fake, True) + opt.sparse_w*sparse_loss + opt.RecZ_w*err_recZ + opt.content_w_all*content_loss_all + opt.content_w_bg*content_loss_bg
...
# The second stage: fix the bg network, train the mask network with the pseudo label
for epoch in range(opt.niter + 1, opt.niter * 5 + 1):
    # train
    for iteration, batch in enumerate(training_data_loader, 1):
        # forward
        noise_vec = torch.randn(1, opt.noise_nc).to(device)
        ...
        for i in range(opt.batch_size):
            ...
            mask_pseudo_label = morphology.remove_small_objects(mask_pseudo_label, min_size=800, connectivity=1)
            mask_pseudo_label = morphology.remove_small_holes(mask_pseudo_label)
    ...
    if epoch%1 == 0:
        ...
        for iteration, (image, label) in enumerate(testing_data_loader, 1):
            with torch.no_grad():
                ...
                combine = torch.cat((image, mask, label), dim=3, out=None)
                combine = torch.squeeze(combine, 1)
                combine = combine.data.cpu().numpy()
                combine = (combine + 1) * 128
                combine = np.reshape(combine, (-1, opt.image_size*3))
                print("===> Testing:[{}]({}/{})".format(epoch, iteration, len(testing_data_loader)))
    ...
    update_learning_rate(net_mask_scheduler, optimizer_mask)
    ...
    if epoch % 50 == 0:
        ...
        net_mask_model_out_path = "{}/netMASK_model_epoch_{}.pth".format(ckpt_path, epoch)
        torch.save(net_mask, net_mask_model_out_path)
        print("Checkpoint saved to {}".format(ckpt_path))

net_mask.eval()


print('===> Loading synthesized datasets')
train_set_syn = get_training_set(opt.dataset_syn, opt.dataset_dir)

training_data_syn_loader = DataLoader(dataset=train_set_syn, num_workers=opt.threads, batch_size=opt.batch_size, shuffle=True)

# The third stage: refining the U-Net with synthesized cell images in USAG
for epoch in range(opt.niter * 5 + 1, opt.niter * 20 + 1):
    # train
    for iteration, (image, label) in enumerate(training_data_syn_loader, 1):
        # forward
        image = image.to(device)
        label = label.to(device)
        mask = net_mask(image)
        ...
        optimizer_mask.zero_grad()
        # calculate the cross entropy loss
        loss_CE = criterionBCE(mask, label)
