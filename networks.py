...
def define_UNet(input_nc, output_nc, ngf, norm='instance', use_dropout=False, init_type='normal', init_gain=0.02, gpu_id='cuda:0'):
    net = None
    norm_layer = get_norm_layer(norm_type=norm)

    net = ResnetUNet(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9)
   
    return init_net(net, init_type, init_gain, gpu_id)


def define_BG(noise_nc, output_nc, init_type='normal', init_gain=0.02, gpu_id='cuda:0'):
    net = None
    net = ResnetBG(noise_nc, output_nc)
...
# Defines the U-Net that consists of Resnet blocks between a few downsampling/upsampling operations.
class ResnetUNet(nn.Module):
    ...
    def forward(self, input_img):
        ######################
        # U-Net for generating mask of the input image
        out = {}
        out['in'] = self.inc(input_img) # 64*256*256
        out['d1'] = self.down1(out['in']) # 128*128*128
        out['d2'] = self.down2(out['d1']) # 256*64*64
        ...
        return self.outc(out['c2'])


# Defines the generator that consists of Resnet blocks for background image generation.
class ResnetBG(nn.Module):
    def __init__(self, noise_nc, output_nc):
        super(ResnetBG, self).__init__()
        self.output_nc = output_nc
    ...
    def forward(self, noise_vec):
        ######################
        # Res-Blocks for generating background image
        x = self.linear(noise_vec)
        x = x.view(-1, 128, 4, 4)
        x = self.layer_up_1(x)
        x = self.layer_up_2(x)
